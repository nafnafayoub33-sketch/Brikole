...
**Business model:** two lines, and the first one is that **being on the list is free**.
A tradesman is listed, searchable and able to send offers without paying anything. The
list of tradesmen *is* the product; charging a man to appear on it makes the product
worse for the client who opens the app and finds three plumbers instead of ten.

1. **5 DH per lead**, charged to the *tradesman* **only when the client and he have
   both agreed the same terms** — that is the moment a real lead was delivered. Taken
   from a prepaid balance, set per trade in `platform_settings`. **The first 5 jobs are
   free.** Balance at zero → he stops seeing the request feed.
2. **20 DH a month, optional, for placement.** It buys a higher position in search and
   a badge that says so. It buys nothing else: not a better rating, not a hidden
   competitor, not a client's trust.

There is **no commission on the job price**, and the platform never holds client money:
the client pays the tradesman directly, in cash. The fee scales with success and the
subscription does not, which is the point of having both — a platform that earns more
when it works less is a platform pointed the wrong way.

## The rule the product is built around

**No contact detail reaches the other side until they have agreed.** The platform
...
- The chat between them (C9 / M12) strikes numbers, emails, links and `@handles` out
  of every message **in both directions**, including numbers spelled in words and
  typed in Arabic-Indic digits. The message is still delivered, with the contact
  struck out; the struck contact is never stored.
- **Nobody is refused and nobody is charged for trying.** He types his number, the
  message goes, and the number is not in it — the bubble says a contact was removed,
  so he is not left waiting for a call that was never coming. Refusing the message
  outright teaches people to write `zero six`; pricing it puts a paywall in the
  middle of a conversation two people are having in good faith.
- **What the platform does instead is count.** Past a threshold of *different
  clients* (`contact_flag_threshold`, default 10) the platform files a report on the
  tradesman's profile, under no name, and staff decide. One client is a man answering
  a question. Ten is a business being run off the back of the platform. Counted in
  clients, never in messages: persistence at one man who is not replying is not a
  pattern.
